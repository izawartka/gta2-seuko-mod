...
bool ModMenuModule::ToggleNativeCheatSegment::SetSegmentData(const ToggleNativeCheatSegmentData& data)
{
	...
	const auto* cheatDef = Utils::CategorizedNativeCheats::GetCheatDef(data.cheatIndex);
	if (!cheatDef) {
...
bool ModMenuModule::ToggleNativeCheatSegment::Attach(ModMenuModule::MenuBase* menu, UiModule::Component* parent)
{
	...
	const auto& cheatCategoriesList = Utils::CategorizedNativeCheats::GetAllCategories();
	...
	m_cheatCategoryController = m_menuController->CreateLatestItemController<UiModule::SelectController<Utils::CategorizedNativeCheats::NativeCheatCategory>>(
		cheatCategoryText,
		cheatCategoriesList,
...
void ModMenuModule::ToggleNativeCheatSegment::CreateCheatController(Utils::CategorizedNativeCheats::NativeCheatCategory category)
{
	if (m_cheatMenuItemId == -1 || !m_cheatText) return;
	if (m_cheatController) return;

	const auto& cheatsInCategory = Utils::CategorizedNativeCheats::GetCheatsByCategory(category);
...
void ModMenuModule::ToggleNativeCheatSegment::OnCheatCategoryControllerSave(Utils::CategorizedNativeCheats::NativeCheatCategory newCategory)
{
	DestroyCheatController();
